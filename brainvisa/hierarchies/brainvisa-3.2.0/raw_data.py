...
insert(
    ...
    'flairmri', SetWeakAttr('modality', 'flairmri'),
    SetContent(
        '{acquisition}',
        SetType('Acquisition'),
        SetDefaultAttributeValue('acquisition', default_acquisition),
        SetNonMandatoryKeyAttribute('acquisition'),
        SetContent(
            '<subject>', SetType('Raw FLAIR MRI'), SetPriorityOffset(+1),
            SetWeakAttr('normalized', 'no'),
            'normalized_{normalization}_<subject>', SetType('Raw FLAIR MRI'),
            SetWeakAttr('normalized', 'yes'),

            'registration',
            SetContent('RawFLAIR-<subject>_<acquisition>', SetType('Referential of Raw FLAIR MRI')),
        ),
    ),
