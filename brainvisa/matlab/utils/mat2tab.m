function mat2tab(A,sformat,nom_fichier,B)
% mat2tab.m	Conversion d'une matrice au format \tabular LaTeX, avec
%		filtrage utilisant \V et \e pour la version VF.
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% mat2tab(A,sformat,nom_fichier,B)
%
...
% Localisation de chaque nombre et isolement entre $ $ 
  ...
  ch = sprintf(ch,A);
  ind = find(ch=='$');		% Filtrage par \e et \V pour la version VF
  for i=length(ind):-2:2,
    sf = ch(ind(i-1):ind(i)-1)
    sf = strsubst(sf,'.','\V');	
    sf = strsubst(sf,'E','e');
    if ~isempty(find(sf=='e'))
      sf = [strsubst(sf,'e','\e{') '}'];
      sf = strsubst(sf,'\e{-0','\e{-');
      sf = strsubst(sf,'\e{+0','\e{');
      sf = strsubst(sf,'\e{+','\e{');
    end
    ch = [ch(1:ind(i-1)-1) sf ch(ind(i):length(ch))];
  end
fprintf(fid,'%s',ch);
end
%
