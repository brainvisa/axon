...
class DatabaseServer(Singleton):
    ...
    def addDatabase(self, database):
        ...
        if os.path.exists(remoteAccessURI):
            print('WARNING: database', repr(database.directory),
                  'has the following remote access:', open(remoteAccessURI).read())
        else:
            if database.sqlDatabaseFile != ':memory:':
                try:
                    ...
                    temporaries.append(
                        temporaryManager.createSelfDestroyed(remoteAccessURI))
                    dbfile.write(str(uri))
                    print('Serving database', repr(
                        database.directory), 'with URI', uri)
                except IOError:
                    print('database', repr(database.directory),
                          'cannot be used with server access')
