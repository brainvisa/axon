...
#==============================================================================
  ...
  def _checkIfValid(self, text):
    ...
    if text == '':
      ...
      self.new_name_line_edit.setStyleSheet('background-color: rgb(255,255,153);')
    else:
...
#==============================================================================
  ...
  def _updateComboBox(self, item, column):
    if item.data(0, Qt.UserRole) is not None:
      ...
      for process_data in process_data_list:
        self.process_name_combo_box.addItem(process_data[0], process_data[1])
    else:
      self.process_name_combo_box.clear()
  ...
  def _loadProcess(self):
    data = self.process_name_combo_box.itemData(self.process_name_combo_box.currentIndex())
    if data is not None:
      process_path = data[1]
      try:
        showProcess(getProcessInstance(process_path))
      except:
        QMessageBox.warning(self, '', "Brainvisa process read error occured")
  ...
  def _buildProcessesTree(self):
    ...
    home_directory = os.path.expanduser('~')
    root_directory = os.path.join(home_directory, ".brainvisa","setting_preferences")
    for root, dirs, files in os.walk(root_directory):
      for proc_file in files:
        ...
        if path.endswith(".bvproc"):
          ...
          directory_without_root = directory.replace(root_directory, '')
    ...
    if directory_list:
      ...
      for child_index in range(current_node.childCount()):
        if process_id == current_node.child(child_index).text(0):
          current_data_list = current_node.child(child_index).data(0, Qt.UserRole)
