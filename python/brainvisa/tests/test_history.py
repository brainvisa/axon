...
class TestDatabaseHistory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dbs.expert_settings.sqliteFileName = os.path.join(
            db_directory, "database.sqlite")
        self.dbs.expert_settings.activate_history = True
        neuroConfig.dataPath.append(self.dbs)
        self.db = SQLDatabase(
            self.dbs.expert_settings.sqliteFileName,  db_directory,
    ...
    def test_store(self):
        ...
        output = wd.findValue(
            {"_database": self.db.name, "center": "mycenter", "subject": "mysubject"})
        defaultContext().runProcess(
            "importT1MRI", self.example_data.name, output)
        loctime = time.localtime()  # WARNING may be too late already
        diskitem = self.db.findDiskItem(
            {"_type": "Raw T1 MRI", "center": "mycenter", "subject": "mysubject"})
