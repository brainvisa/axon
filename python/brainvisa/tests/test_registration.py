...
class TestTransformationManager(unittest.TestCase):
  ...
  def setUp(self):
    ...
    for s in xrange( 1, self.number_of_subjects + 1 ):
      self.create_referential( registration_directory, 'Subject ' + str( s ), s )
      self.create_transformation( registration_directory, ( 1, 0, 0, 0, s ), s, 0 )
      for s2 in xrange( s+1, self.number_of_subjects + 1 ):
        self.create_transformation( registration_directory, ( 1, 0, 0, s, s2 ), s, s2 )
      for n in xrange( 1, self.number_of_sulci + 1 ):
        self.create_referential( registration_directory, 'Sulci ' + str( n ) + ' of subject ' + str(s), ( s, n  ) )
        self.create_transformation( registration_directory, ( 1, 0, 1, s, n ), s, ( s, n ) )
