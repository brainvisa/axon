...
class AxonFedjiDatabase(Database):
    def __init__( self, fedji_url, fedji_database, directory, fso=None, context=None ):
        super(AxonFedjiDatabase, self).__init__()
        self.fedji_collection = fedji_connect(fedji_url).axon.disk_items
        self.name = fedji_database
    ...
    def insertDiskItems( self, diskItems, update=False ):
        for d in diskItems:
            if d.type is None:
                raise DatabaseError( _('Cannot insert an item wthout type in a database: %s') % ( unicode( d ), ) )
            ...
            if update:
                ...
                attributes = d.globalAttributes()
                attributes['type'] = [d.type.name] + [i.name for i in d.type.parents()]
                attributes['format'] = d.format.name
                attributes['_id'] = attributes.pop('uuid')
                attributes['files'] = d._files
                self.fedji_collection.insert(attributes)
  ...
  def findAttributes( self, attributes, selection={}, _debug=None, exactType=False, **required ):
    #print "*findAttributes ", attributes
    #types = set( self.getAttributeValues( '_type', selection, required ) )
    #required["_type"] = types
    
    #print "Find Attributes ", attributes
    try:
