...
def mri_path(attributes):
    ...
    acquisition = '_'.join(i for i in (attributes.get('visit'),
                                        attributes.get('scan'),
                                        '-'.join(attributes.get('bias_correction',[])),
                                        '-'.join(attributes.get('geo_correction',[])),
    ...
    path.append(attributes['subject'] + (attributes['format'] if 'format' in attributes else '.nii'))
...
for i in xrange(nb_subjects):
    ...
    attributes = dict(study=study, center=center, subject=subject, modality='pet')
    tracers = ['fdg', 'datscan']
    for visit in visits:
        if visit:
            attributes['visit'] = visit
