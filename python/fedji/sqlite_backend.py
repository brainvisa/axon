...
class_to_field_type = dict((i, eval(j))
                           for i, j in six.iteritems(class_to_class_name))
    ...
    def connection(self):
        if self._connection is None:
            if not osp.exists(self.db):
                ...
                if not osp.exists(d):
                    os.makedirs(d)
            cnx = self._connection = sqlite3.connect(
...
class FedjiSqliteCollection(object):
    ...
    def new_field(self, field, cls):
        ...
        if field not in self.fields:
            ...
            if atype == 'list':
                ...
                cnx.execute(sql)
                cnx.execute('CREATE INDEX idx_list_%s ON %s (list, value)' %
                            (field, list_table))
    ...
    def remove(self, query):
        ...
        for field, type in six.iteritems(self.fields):
            if type is list:
                list_tables.append(
                    '%s_list_%s' % (self._documents_table, field))
    ...
    def drop(self):
        cnx = self._connect()
        for field, type in six.iteritems(self.fields):
            cnx.execute('DROP INDEX IF EXISTS %s_%s' %
                        (self._documents_table, field))
            if type is list:
                list_table = '%s_list_%s' % (self._documents_table, field)
...
class FedjiSqliteQueryResult(object):
    ...
    def _get_sql_operator_has(self, field, value, inner_join, where, where_data):
        ...
        if field_type is list:
            list_table = '%s_list_%s' % (
                self.collection._documents_table, field)
            inner_join.append(' INNER JOIN %(list_table)s ON %(data_table)s.rowid = %(list_table)s.list' %
                              {'data_table': self.collection._documents_table, 'list_table': list_table})
    ...
    def _get_sql_operator_in(self, field, value, inner_join, where, where_data):
        ...
        if field_type is list:
            ...
            inner_join.append(' INNER JOIN %(list_table)s ON %(data_table)s.rowid = %(list_table)s.list' %
                              {'data_table': self.collection._documents_table, 'list_table': list_table})
            where.append('%s.value IN (%s)' %
                         (list_table, ','.join(('NULL' if i is None else '?') for i in values)))
    ...
    def _get_sql(self, select):
        ...
        for field, value in six.iteritems(self.query):
            if isinstance(value, dict):
                if len(value) != 1:
                    raise ValueError(
                        'A dictionary with %d item(s) is not a valid query value.' % len(value))
                operator, value = value.popitem()
            else:
                operator = '$has'
            operator_method = None
            if operator.startswith('$'):
                operator_method = getattr(
                    self, '_get_sql_operator_%s' % operator[1:], None)
            if operator_method is None:
                raise ValueError(
                    '%s is not a valid query operator for FEDJI' % str(operator))
    ...
    def _rowids(self):
        ...
        '''
        cnx = self.collection._connect()
        sql, sql_data = self._get_sql(
            'SELECT %s.rowid' % self.collection._documents_table)
        if sql:
            for row in cnx.execute(sql, sql_data):
                yield row[0]
