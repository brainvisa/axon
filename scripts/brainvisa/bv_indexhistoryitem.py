...
parser = OptionParser( description='Parse BV history items and inserts outputs of processes that have run into the database. Databases need to be already setup in BrainVisa.' )
parser.add_option( '-i', '--input', dest='infile', help='input history file (.bvproc)', action='append', default=[] )
parser.add_option( '-d', '--dir', dest='dir', help='input history_book directory' )
parser.add_option( '-n', '--newer', dest='newer', help='read only items newer than <date> in a history_book directory. format: 2012/11/15 or 2012/11/15 18:41:00' )
parser.add_option( '-r', '--remove', dest='remove', action='store_true', help='remove obsolete history files (referencing only output files that have been rewritten by newer histoty files)' )
parser.add_option( '-s', '--simulation', dest='simulation', action='store_true', help='Simulation mode (dry run). Don\'t insert elements in the databases nor remove obsolete history files. Just parse files and look at what should have to be done.' )
